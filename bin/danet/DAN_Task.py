...
from bin.danet.lib.utils import PredictDataset
from bin.danet.abstract_model import DANsModel
from bin.danet.lib.multiclass_utils import infer_output_dim, check_output_dim
...
class DANetClassifier(DANsModel):
    ...
    def compute_loss(self, y_pred, y_true, reduction='mean'):
        return self.loss_fn(y_pred, y_true.long(), reduction=reduction)
    ...
    def update_fit_params(
        ...
        self.mi_scores = torch.from_numpy(mi_scores / mi_scores.sum()).float().to(self.device)
